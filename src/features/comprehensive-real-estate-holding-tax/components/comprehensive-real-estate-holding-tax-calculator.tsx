...
import {
  calculateComprehensiveRealEstateHoldingTax,
  type ComprehensiveRealEstateHoldingTaxResult,
} from "../calculate";
import {
...
import {
  ...
  type ComprehensiveRealEstateHoldingTaxErrors,
...
export function ComprehensiveRealEstateHoldingTaxCalculator({
  locale,
}: {
  ...
  const placeholders: ComprehensiveRealEstateHoldingTaxValues = {
  ...
  return (
    <section aria-labelledby="comprehensive-real-estate-holding-tax-input-title">
      <div className={dashboardCalculatorWorkspaceClass}>
        ...
        >
          ...
          <h2
            id="comprehensive-real-estate-holding-tax-input-title"
          ...
          {fields.map((key) => {
            const money = moneyFields.has(key);
            return (
              <Field
                key={key}
                id={key}
        ...
        <div className="min-w-0 space-y-4">
          <section
            ref={resultRef}
            aria-labelledby="comprehensive-real-estate-holding-tax-result"
            className="scroll-mt-20 rounded-xl border bg-card p-4 shadow-sm"
          >
            <h2
              id="comprehensive-real-estate-holding-tax-result"
            ...
            <PrimaryResults
              metrics={[
                ...
                {
                  label: copy.comprehensiveTax,
                  value: (
                    <AnimatedWon
                      value={result?.comprehensiveTax ?? null}
                      animationKey={animationKey}
                    />
                  ),
                },
          ...
          <details open className="rounded-xl border bg-card p-4 shadow-sm">
            ...
            {result ? (
              <dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2">
                <Detail
                  label={copy.amountAfterDeduction}
                  value={won(result.amountAfterDeduction, locale)}
                />
                <Detail
