...
import { calculateGiftTax, type GiftTaxResult } from "../calculate";
...
const initialValues: GiftTaxValues = {
  giftValue: "",
  assumedDebt: "",
  deductibleAmount: "",
  taxRate: "",
...
export function GiftTaxCalculator({ locale }: { locale: GiftTaxLocale }) {
  ...
  const [errors, setErrors] = useState<GiftTaxErrors>({});
  ...
  return (
    <section aria-labelledby="gift-tax-input-title">
      <div className={dashboardCalculatorWorkspaceClass}>
        ...
        >
          ...
          <h2 id="gift-tax-input-title" className="mt-1 text-xl font-semibold">
            {copy.input}
          </h2>
        ...
        <div className="min-w-0 space-y-4">
          <section
            ref={resultRef}
            aria-labelledby="gift-tax-result"
            className="scroll-mt-20 rounded-xl border bg-card p-4 shadow-sm"
          >
            <h2 id="gift-tax-result" className="text-xl font-semibold">
              {copy.result}
            </h2>
            <PrimaryResults
              metrics={[
                ...
                {
                  label: copy.netReceived,
                  value: (
                    <AnimatedWon
                      value={result?.netReceived ?? null}
                      animationKey={animationKey}
                    />
                  ),
                },
                {
                  label: copy.taxableGift,
                  value: (
                    <AnimatedWon
                      value={result?.taxableGift ?? null}
                      animationKey={animationKey}
                    />
                  ),
                },
          ...
          <details open className="rounded-xl border bg-card p-4 shadow-sm">
            ...
            {result ? (
              <dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2">
                <Detail
                  label={copy.netGift}
                  value={won(result.netGift, locale)}
                />
                <Detail
