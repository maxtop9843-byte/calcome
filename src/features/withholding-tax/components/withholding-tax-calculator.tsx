...
import {
  ...
  type WithholdingTaxErrors,
  type WithholdingTaxValues,
} from "../validation";
...
const initialValues: WithholdingTaxValues = {
  grossPayment: "",
  nonTaxableAmount: "",
  nationalTaxRate: "",
  localIncomeTaxRate: "",
...
export function WithholdingTaxCalculator({
  ...
  const [errors, setErrors] = useState<WithholdingTaxErrors>({});
  ...
  return (
    <section aria-labelledby="withholding-tax-input-title">
      <div className={dashboardCalculatorWorkspaceClass}>
        ...
        >
          ...
          <h2
            id="withholding-tax-input-title"
        ...
        <div className="min-w-0 space-y-4">
          <section
            ref={resultRef}
            aria-labelledby="withholding-tax-result"
            className="scroll-mt-20 rounded-xl border bg-card p-4 shadow-sm"
          >
            <h2 id="withholding-tax-result" className="text-xl font-semibold">
              {copy.result}
            </h2>
            <PrimaryResults
              metrics={[
                {
                  label: copy.totalWithholding,
                  value: (
                    <AnimatedWon
                      value={result?.totalWithholding ?? null}
                ...
                {
                  label: copy.netPayment,
                  value: (
                    <AnimatedWon
                      value={result?.netPayment ?? null}
                      animationKey={animationKey}
                    />
                  ),
                },
                {
                  label: copy.taxablePayment,
                  value: (
                    <AnimatedWon
                      value={result?.taxablePayment ?? null}
                      animationKey={animationKey}
                    />
                  ),
                },
