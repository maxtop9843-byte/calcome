...
import {
  validatePropertyTax,
  type PropertyTaxErrors,
  type PropertyTaxValues,
} from "../validation";
...
export function PropertyTaxCalculator({
  ...
  const [errors, setErrors] = useState<PropertyTaxErrors>({});
    ...
    <section aria-labelledby="property-tax-input-title">
      <div className={dashboardCalculatorWorkspaceClass}>
        ...
        >
          ...
          {fields.map((key) => {
            const money =
              key === "assessedValue" || key === "progressiveDeduction";
            const placeholders = {
        ...
        <div className="min-w-0 space-y-4">
          <section
            ref={resultRef}
            aria-labelledby="property-tax-result"
            className="scroll-mt-20 rounded-xl border bg-card p-4 shadow-sm"
          >
            <h2 id="property-tax-result" className="text-xl font-semibold">
              {copy.result}
            </h2>
            <PrimaryResults
              metrics={[
          ...
          <details open className="rounded-xl border bg-card p-4 shadow-sm">
            ...
            {result ? (
              <dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2">
                <Detail
                  label={copy.cityAreaTax}
                  value={won(result.cityAreaTax, locale)}
                />
                <Detail
